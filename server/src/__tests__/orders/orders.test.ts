...
describe("Order Placement API", () => {
  ...
  describe("POST /api/orders", () => {
    ...
    it("should clear cart after successful order", async () => {
      ...
      const cartResponse = await request(app).get(`/api/cart/${testSessionId}`);
  ...
  describe("GET /api/orders/session/:sessionId", () => {
    ...
    it("should support pagination for session orders", async () => {
      const response = await request(app).get(
        `/api/orders/session/${testSessionId}?page=1&limit=1`,
      );
    ...
    it("should use default pagination for session orders", async () => {
      const response = await request(app).get(
        `/api/orders/session/${testSessionId}`,
      );
